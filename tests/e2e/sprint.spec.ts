...
for (const action of ['next', 'home']) {
  test(`40LINE shows only the final time and allows ${action === 'next' ? 'another sprint' : 'saving the completed replay from My Page'}`, async ({
    page,
  }) => {
    ...
    await expect(page.locator('#result-title')).toHaveText('00:15.616');
    for (const selector of ['#result-eyebrow', '#result-description', '#result-stats'])
      await expect(page.locator(selector)).toBeHidden();
    await expect(page.locator('#result-dialog')).not.toContainText('リプレイ保存');
    ...
    if (action === 'home') {
      ...
      const replay = new ReplayPlayer(
        parseReplay(await readFile((await download.path())!, 'utf8')),
      );
      while (!replay.done) replay.step();
      expect(replay.valid).toBe(true);
      expect(replay.match.roundTicks).toBe(fixture.roundTicks);
      expect(replay.match.winner).toBe(0);
      return;
    }
    await page.evaluate(() => {
