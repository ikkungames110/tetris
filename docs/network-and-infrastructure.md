...
- `apps/web/matchmaking.ts` は、プロトコル・ルール版ごとの共有Peer IDを1台の待機ブラウザーが取得する。ID使用中ならそのブラウザーへ接続し、版を検証した `hello / offer / accept` で通常のルームコードを交換する。2人の入室後に自動で準備完了を送り、試合開始時に待機Peerを破棄して次の組へIDを解放する。
- マッチング候補は1人だけ予約し、競合した参加者は短いランダム待機後に再試行する。キャンセル・切断・交換タイムアウト時は候補と待機IDを破棄する。接続失敗が続く場合は再試行可能なエラーに戻す。Peer一覧公開APIや追加データベースは不要。[PeerJSのID指定・エラー・破棄](https://peerjs.com/client/api/peer)
- このマッチングは小規模向け。レート・地域・厳密な待ち順・悪意ある共有IDの占有への耐性は保証しない。P2P接続にTURNが必要な回線ではマッチングにもTURNが必要。
- `packages/network/rooms.ts` にルーム管理・判定を共通化。Node.js専用APIを取り除き、ホストのブラウザーが同じ60Hzのエンジンを実行する。ホストの手元へは60Hz、相手に送る盤面は通常30Hz、固定・攻撃・フェーズ変更時も配信する。
- 通信プロトコル版7では、追加5種類のテンプレートIDと5番目のSRSキックを受け付ける。新しいIDを検証できない旧クライアントとは接続・待機列を分ける。版6で追加した先取本数と両者の表示名も配信する。先取本数は共通エンジンにルームごとのルールとして渡し、受信時にスコアの上限・表示名の長さを検証する。
- RTCDataChannelは順序付き・信頼性あり。受信する操作のサイズ／頻度／版／試合ID／連番を検証する。参加者はホストのスナップショットを検証する。盤面400セルは1セル1文字にまとめて送る（プロトコル版2）。
- `packages/network/prediction.ts` で参加者の自盤面だけを先行表示。60Hzの入力へ連番を付け、ホストのACKより新しい入力を確定盤面へ再適用する。既知のNEXTだけを使い、未公開ピース・おじゃまの穴・勝敗は予測しない。回線遅延や攻撃の到着による盤面補正は残る。
- 入力の押下エッジを順序通り処理し、ジッターで溜まった同一保持状態の冗長フレームは圧縮する。250ms入力が来なければ保持解除。再接続・ラウンド変更で入力履歴を破棄する。
- ランダム対戦は3本先取。ルーム対戦は作成時に1〜9本（初期値3本）から選ぶ。各ラウンドは盤面上のWIN / LOSEを3秒表示後、次ラウンドをホストが開始する。指定本数の決着時だけ結果ダイアログを出し、ルーム対戦では同じ部屋で双方の準備完了を待つ。切断中は自動進行を止め、復帰後に3秒を取り直す。開始前の3秒カウントダウンは維持する。
- Canvasは盤面・アクティブミノ・消去エフェクトが変わったときだけ描画。HOLD/NEXT、テキスト、ルームUIの不要な更新も省く。
- ホストはページを開いたままにする。ブラウザー停止から復帰すると、古い入力を大量に進めず処理を再開する。放置やアプリのping無応答だけで切断しない。ホスト交代・状態の永続化はない。参加者だけは10秒以内の再接続・再読み込み復帰が可能。
- P2Pのホストは判定を書き換えられるため、悪意あるホストに対する公平性は保証しない。招待対戦と小規模なランダムマッチングが対象。
...
- Origin検証（既定はHost一致、配信時は許可リストを設定可能）、2KBの受信上限、1接続180メッセージ/秒、送信滞留512KBで切断。最大100ルーム・220接続。接続中の待機／結果画面に時間制限は設けない。これらは負荷保証値ではない。
...
状態遷移は `waiting → ready → countdown → playing → roundResult → countdown / matchResult → ready`（ルーム対戦）。退室・実際の通信切断による終了は `closed`。ルール変更は waiting 中だけ許す。参加チケットはユーザー・ルーム・席に結び付け、部屋番号だけで席を奪えないようにする。
