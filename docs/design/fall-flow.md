...
既存の`landing()`の結果から、着地点の各セルに薄い塗りと枠線のゴーストを描く。
ゴーストにはピースの色と透明度設定を反映する。

### HUD・操作

- STOCK（保管）は盤面左上、QUEUE（次のピース）は盤面右上に置く。
- QUEUEは縦方向に5個を上から順に表示する。両プレビューのミノは横幅を十分に使って大きく描く。
...
`tests/e2e/design.spec.ts`で上部HUD、最小タップ領域、画面内の配置、7種×4回転×空／段差盤面の描画とゴーストを確認する。
