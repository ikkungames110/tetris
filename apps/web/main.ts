...
import './style.css';
import './arena.css';
import { ADS_ENABLED, desktopAdRail, mountAds } from './ads';
...
$('#app').innerHTML = `
  ...
  ${ADS_ENABLED ? desktopAdRail('left') : ''}
  <main>
  ...
  ${ADS_ENABLED ? desktopAdRail('right') : ''}
  </div>
  ...
  ${ADS_ENABLED ? `<aside class="bottom-ad" aria-label="画面下部のi-mobile広告"><div class="bottom-ad-row"><div class="ad-slot" data-ad="bottom" aria-label="PC用i-mobile広告"></div><div class="ad-slot" data-ad="mobile" aria-label="スマホ用i-mobile広告"></div></div></aside>` : ''}
  ${rankingsHTML}
    ...
    if (ADS_ENABLED) {
      ...
      document.body.style.setProperty(
        ...
        String(
          Math.min(
            1,
            (available - 8) / $('.ad-rail').offsetHeight,
            parseFloat(getComputedStyle(layout).gridTemplateColumns) / 468,
          ),
        ),
      );
    }
...
async function loadRooms(): Promise<void> {
  if (roomListLoading || Date.now() < roomListRefreshAt) return;
  ...
  roomListRefreshAt = Date.now() + 15_000;
  updateRoomRefresh();
  ...
  try {
    ...
    for (const room of data.rooms) {
      ...
      button.setAttribute('aria-label', `${room.name}のルームを選択`);
      button.onclick = () => {
      ...
      password.textContent = room.locked ? 'あり' : 'なし';
      const wins = document.createElement('td');
      wins.textContent = `${room.winsRequired}本先取`;
      row.append(host, password, wins);
