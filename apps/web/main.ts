...
import { getPalette, PALETTES, setPalette } from './palettes';
import {
  BOARD_ROWS,
  clearLabel,
  drawBoard,
...
function resizeMobileBoard(): void {
  ...
  const bottom = landscape
    ? ADS_ENABLED
      ? $('.mobile-ad').getBoundingClientRect().top
...
function previewSkin(): void {
  for (const [i, piece] of (['I', 'J', 'L', 'O', 'S', 'T', 'Z'] as const).entries())
